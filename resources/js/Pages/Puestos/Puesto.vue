        ...
        <div class="q-pa-md">
            <q-card class="my-card">
                <q-card-section class="ml-6">
                    ...
                    <div class="text-subtitle">Registro de los puestos de trabajo de la organizacion.</div>
                </q-card-section>
                <q-card-section>
                    <div class="row">
                        <div class="col-12 col-md-8">
                            <q-item>
                                <q-input filled bottom-slots v-model="puesto.nombre" class="full-width" label="Nombre:"
                        ...
                        <div class="col-12 col-md-4">
                            <q-item>
                                <q-input filled bottom-slots v-model="puesto.nro_plazas" type="number" class="full-width"
                                    label="Numero de plazas:" :error-message="errores.nro_plazas && errores.nro_plazas[0]"
...
const guardar = async () => {
    ...
    //Guardar puesto
        ...
        await axios.post("/api/puestos_agregar", puesto.value).then((response) => {
            reiniciarValores()
            $q.notify(
                {
                    type: 'positive',
...
const eliminar = async () => {
    await axios.post("/api/puestos_eliminar/" + puesto.value.id).then((response) => {
        reiniciarValores()
        $q.notify(
            {
                type: 'positive',
