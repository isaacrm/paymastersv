<template>
  <AppLayout title="Planillas">
    <div
      ...
      v-show="
        valoresExistentes.cantidadEmpleados <= 0 ||
        valoresExistentes.cantidadDescuentos <= 0
      "
    >
      <q-banner
        inline-actions
        class="text-white bg-red"
        v-show="valoresExistentes.cantidadEmpleados <= 0"
      >
      ...
      <q-banner
        inline-actions
        class="text-white bg-red"
        v-show="valoresExistentes.cantidadDescuentos <= 0"
      >
        ...
        <template v-slot:body-cell-operaciones="props">
          <q-td :props="props">
            <q-btn
              ...
              icon="table_view"
              class="mr-2"
              @click="redireccionDetalle(props.row.id)"
            ></q-btn>
            <q-btn
              round
              color="warning"
...
const comprobacionInicial = async () => {
  await axios
    ...
    .then((response) => {
      valoresExistentes.value.cantidadDescuentos =
        response.data.descuentosExistentes;
      valoresExistentes.value.cantidadEmpleados =
        response.data.empleadosExistentes;
    })
    .catch((error) => {
      errored.value = true;
    });
