      ...
      <q-card class="my-card">
        <q-card-section class="ml-6">
          ...
          <div class="text-subtitle">
            Registro de las {{ nombres.minu + "es" }} de trabajo de la
            organizacion.
          </div>
        </q-card-section>
        <q-card-section>
          ...
          <div class="row">
            <div class="col-12 col-md-6">
              <q-item>
                <q-select
                  v-model="datosSelect.superior_id"
            ...
            <div class="col-12 col-md-6">
              <q-item>
                <q-select
                  v-model="datosSelect.centro_de_costos"
                  label="Seleccione un centro de costos"
          ...
          <div class="row">
            <div class="col-12 col-md-12">
              <q-item>
                <q-select
                  v-model="datosSelect.nivel_organizacional"
                  label="Seleccione el superior en el nivel organizacional"
                  :options="unidades"
                  option-label="name"
                  option-value="id"
                  class="full-width"
...
const columns = [
  ...
  },
  {
    ...
    field: "centro_costos_nombre",
  },
  {
    name: "nivel_organizacional_nombre",
    align: "left",
    label: "Nivel organizacional",
    field: "nivel_organizacional_nombre",
  },
...
const guardar = async (user_id) => {
  ...
  // Actualizar
  if (datosSelect.value.centro_de_costos) {
    datos.value.centro_de_costos = datosSelect.value.centro_de_costos.id;
  }
  if (datosSelect.value.superior_id) {
    datos.value.superior_id = datosSelect.value.superior_id.id;
  }
  if (datosSelect.value.nivel_organizacional) {
    datos.value.nivel_organizacional =
      datosSelect.value.nivel_organizacional.id;
  }
  ...
  //Guardar datos
    ...
    await axios
      .post(`/api/${nombres.minu}es_agregar`, datos.value)
      .then((response) => {
        reiniciarValores();
        $q.notify({
          type: "positive",
...
const editar = (editardatos) => {
  ...
  datosSelect.value.superior_id = {
    id: editardatos.superior_id,
    name: editardatos.superior_nombre,
  };
  datosSelect.value.centro_de_costos = {
    id: editardatos.centro_costos_id,
    name: editardatos.nombre,
  };
  datosSelect.value.nivel_organizacional = {
    id: editardatos.nivel_organizacional,
    name: editardatos.nivel_organizacional_nombre,
  };
...
const eliminar = async (user_id) => {
  datos.value.user_id = user_id;
  await axios
    .post(`/api/${nombres.minu}es_eliminar/` + datos.value.id, datos.value)
    .then((response) => {
      reiniciarValores();
      $q.notify({
        type: "positive",
...
const obtenerCentroDeCostos = async () => {
  await axios
    .get("/api/centro_de_costos_consultar_centro_de_costos")
    .then((response) => {
