                ...
                <q-card-section>
                    <div class="row">
                        ...
                        <div class="col-12 col-md-4">
                            <q-item>
                                <q-input filled bottom-slots v-model="departamento.codigo_iso" class="full-width" 
                                    label="Código" :error-message="errores.codigo_iso && errores.codigo_iso[0]"
...
const columns = [
    ...
    { name: 'codigo_iso', align: 'left', label: 'Código', field: 'codigo_iso', sortable: true },
...
const guardar = async () => {
    ...
    //Guardar departamento
        ...
        await axios.post("/api/agregar_departamento", departamento.value).then((response) => {
                reiniciarValores()
                $q.notify(
                    {
                        type: 'positive',
...
const eliminar = async () => {
    await axios.post("/api/eliminar_departamento/" + departamento.value.id).then((response) => {
            reiniciarValores()
            $q.notify(
                {
                    type: 'positive',
