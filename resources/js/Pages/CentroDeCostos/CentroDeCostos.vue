        ...
        <div class="q-pa-md">
            <q-card class="my-card">
                <q-card-section class="ml-6">
                    ...
                    <div class="text-subtitle">Registro de los {{ nombres.nombreMinuscula }} de trabajo de la organizacion.
                    </div>
                </q-card-section>
                <q-card-section>
                    <div class="row">
                        <div class="col-12 col-md-4">
                            <q-item>
                                <q-select filled bottom-slots class="full-width" v-model="datos.mes_del" :options="meses"
                                    label="Mes desde" />
                            </q-item>
                        </div>
                        <div class="col-12 col-md-4">
                            <q-item>
                                <q-select filled bottom-slots class="full-width" v-model="datos.mes_al" :options="meses"
                                    label="Mes hasta" />
                            </q-item>
                        </div>
                        <div class="col-12 col-md-4">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-6">
                            <q-item>
                                <q-input filled bottom-slots v-model="datos.presupuesto_inicial" type="number"
                                    class="full-width" label="Presupuesto inicial:"
                                    :error-message="errores.presupuesto_inicial && errores.presupuesto_inicial[0]"
                                    :error="hayError(errores.presupuesto_inicial)" prefix="$" />
                            </q-item>
                        </div>
                        <div class="col-12 col-md-6">
                            <q-item>
                                <q-input filled bottom-slots v-model="datos.presupuesto_restante" type="number"
                                    class="full-width" label="Presupuesto restante:"
                                    :error-message="errores.presupuesto_restante && errores.presupuesto_restante[0]"
                                    :error="hayError(errores.presupuesto_restante)" prefix="$" />
                            </q-item>
                        </div>
                    </div>
                </q-card-section>
...
const columns = [
    ...
    { name: 'mes_del', align: 'left', label: 'Desde el mes:', field: 'mes_del', sortable: true },
    { name: 'mes_al', align: 'left', label: 'Hasta el mes:', field: 'mes_al', sortable: true },
    { name: 'anyo', align: 'left', label: 'Año', field: 'anyo', sortable: true },
    { name: 'presupuesto_inicial', align: 'left', label: 'Presupuesto inicial', field: 'presupuesto_inicial', sortable: true },
    { name: 'presupuesto_restante', align: 'left', label: 'Presupuesto restante', field: 'presupuesto_restante', sortable: true },
...
const guardar = async () => {
    ...
    //Guardar datos
        ...
        await axios.post(`/api/${nombres.minu + "s"}_agregar`, datos.value).then((response) => {
            reiniciarValores()
            $q.notify(
                {
                    type: 'positive',
...
const eliminar = async () => {
    await axios.post(`/api/${nombres.minu + "s"}_eliminar/` + datos.value.id).then((response) => {
        reiniciarValores()
        $q.notify(
            {
                type: 'positive',
